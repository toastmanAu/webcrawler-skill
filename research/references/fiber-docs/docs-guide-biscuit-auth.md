...
Client Request: The client sends an RPC request and includes the Biscuit token in the Authorization HTTP header, formatted as a Bearer token. The token itself is Base64-encoded.

Middleware Interception: On the server, the BiscuitAuthMiddleware intercepts every incoming RPC request before it reaches the actual method handler.
...
Link to file: crates/fiber-lib/src/rpc/biscuit.rs
...
Understanding Payment Channel Network - A Quick Walkthrough Guide